...
def upload_file(filename: str, remote_path: str | None = None) -> str:
    ...
    get_storage_client().Bucket(settings.S3_BUCKET).upload_file(
        Filename=filename,
        Key=remote_path,
        ExtraArgs={"ACL": "public-read", "ContentType": mime_type[0]},
    )
    return f"{settings.S3_PUBLIC_URL}/{remote_path}"
...
def fetch_audio(audio_url: str) -> str:
    ...
    if audio_url.startswith("data:"):
        ...
        if audio_url.startswith(localhost_url):
            audio_url = audio_url.replace(localhost_url, settings.S3_ENDPOINT)
