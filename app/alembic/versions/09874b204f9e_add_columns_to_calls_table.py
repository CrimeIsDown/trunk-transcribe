...
def upgrade() -> None:
    ...
    op.add_column(
        CALLS_TABLE_NAME, sa.Column("start_time", sa.TIMESTAMP(timezone=True), nullable=True)
    )
    op.add_column(CALLS_TABLE_NAME, sa.Column("transcript_plaintext", sa.Text(), nullable=True))
    op.execute(
        f"""
